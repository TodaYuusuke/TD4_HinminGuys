...
void LWP::Utility::FrameTimer::Start()
{
	// 各種項目のリセット
	nowFlame_ = 0.0f;	// フレーム
...
void LWP::Utility::FrameTimer::Start(float time)
{
	// 項目リセット
	Start();
	// 終了秒数取得
...
bool LWP::Utility::FrameTimer::Update()
{
	// 終了している場合
	if (isFinish_) {
		// 実行中ではない
...
void LWP::Utility::FrameTimer::Stop()
{
	// 全メンバ変数をリセット
	nowFlame_		= 0.0f;
	isFinish_		= true;
...
void LWP::Utility::FrameTimer::DebugGUI(const std::string& name)
{
	// ツリーノード開始
...
float LWP::Utility::FrameTimer::GetProgress() const
{
	// 現在のタイマーの進行度を返す
