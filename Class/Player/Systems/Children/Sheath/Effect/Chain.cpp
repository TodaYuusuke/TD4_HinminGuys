...
void Chain::Update() {
	...
	for (PointMass& pointMass : pointMassList_) {
		if (pointMass.model.worldTF.translation.y <= 0.1f) {
			pointMass.model.worldTF.translation.y = std::clamp<float>(pointMass.model.worldTF.translation.y, 0.1f, 10000.0f);
		}
	}
}
