...
DeadSmoke::DeadSmoke(const std::string& texName) {
	texName_ = texName;
}

void DeadSmoke::Update() {
...
void DeadSmoke::Create(const LWP::Math::Vector3& pos) {
	// パーティクルの種類
...
#pragma region 平面に反映
	// 色
	plane_.material.color = jsonData_.colorLimit.min;
	// 座標
...
void DeadSmoke::UpdateParticle() {
	...
	// 色のイージング
	Vector4 color = {
		LerpF((unsigned char)jsonData_.colorLimit.min.R, (unsigned char)jsonData_.colorLimit.max.R, Easing::OutExpo(particleData_.currentTime / particleData_.lifeTime)),
		LerpF((unsigned char)jsonData_.colorLimit.min.G, (unsigned char)jsonData_.colorLimit.max.G, Easing::OutExpo(particleData_.currentTime / particleData_.lifeTime)),
		LerpF((unsigned char)jsonData_.colorLimit.min.B, (unsigned char)jsonData_.colorLimit.max.B, Easing::OutExpo(particleData_.currentTime / particleData_.lifeTime)),
		LerpF((unsigned char)jsonData_.colorLimit.min.A, (unsigned char)jsonData_.colorLimit.max.A, Easing::OutExpo(particleData_.currentTime / particleData_.lifeTime))
	};
