...
void LockOnCamera::RotateUpdate() {
	...
	lockOnOffset_.y = followCamera_->defaultTargetDist_.y + (2.0f * (lockOnTargetDist / followCamera_->maxLength));
	lockOnOffset_.z = followCamera_->defaultTargetDist_.z - (3.0f * (lockOnTargetDist / followCamera_->maxLength));
	...
	// 角度制限
	followCamera_->ClampAngle(isClampAngle, ((followCamera_->GetLockOnData().targetTransform->GetWorldPosition()) - followCamera_->GetCamera()->worldTF.translation).Normalize(), LWP::Utility::DegreeToRadian(followCamera_->kOriginRotateX + 0.0f), LWP::Utility::DegreeToRadian(followCamera_->kOriginRotateX + 80.0f));
